  ...
  <div class="space-y-5">
    <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div class="max-w-3xl">
          <p class="text-sm font-medium text-slate-500">Visió global</p>
      ...
      <div class="mt-6">
        <div class="flex items-end justify-between gap-4">
          <div>
            <p class="text-sm font-medium text-slate-500">Progrés del repartiment</p>
            <p class="mt-1 text-3xl font-semibold text-slate-950">{{ pctGlobal }}%</p>
          </div>
        ...
        <div class="mt-3 h-3 overflow-hidden rounded bg-slate-100">
          <div class="h-full rounded-sm bg-[#00BF33]" :style="{ width: `${pctGlobal}%` }" />
        </div>
      </div>
    </section>
    ...
    <section class="grid grid-cols-2 gap-3 xl:grid-cols-4">
      <div
        v-for="card in cards"
        :key="card.label"
        class="metric-card border-t-4 text-slate-900"
        :class="card.class"
      >
    ...
    <section class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <div class="grid gap-3 md:grid-cols-3">
        <div
          ...
          :class="pas.actiu ? 'border-l-[#0024B6]' : 'border-l-slate-200'"
        >
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <p class="mt-1 text-sm text-slate-500">{{ pas.text }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
      ...
      <div v-if="carregant" class="p-10 text-center text-sm text-slate-500">
      ...
      <div v-else-if="!departamentsOrdenats.length" class="p-10 text-center">
        ...
        <p class="mt-1 text-sm text-slate-500">Sincronitza amb Google Sheets per crear-los.</p>
      </div>
      ...
      <div v-else class="divide-y divide-slate-100">
        ...
        >
          ...
          <div
            ...
            class="mt-4 border border-[#FF8040]/30 bg-[#FF8040]/5 p-4"
          >
            <div class="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
              ...
              >
                ...
                <p class="mt-0.5 text-slate-600">{{ c.materia }} · {{ c.hores }}h</p>
              </div>
            </div>
          </div>
        </article>
...
function estatBadgeClass(dept) {
  if (dept.total === 0) return 'bg-slate-100 text-slate-500';
  if (dept.pct === 100) return 'bg-[#00BF33]/20 text-slate-800';
  if (dept.pct === 0) return 'bg-[#FF8040]/15 text-slate-800';
...
function barraClass(dept) {
  if (dept.pct === 100) return 'bg-[#00BF33]';
...
function puntClass(dept) {
  if (dept.pct === 100) return 'bg-[#00BF33]';
  if (dept.pct === 0) return 'bg-[#FF8040]';
  return 'bg-amber-400';
}
</script>
