...
import { setGuardiesTeacherGuardCount } from '../../../src/services/guardiesStorage.js';
  ...
  <details id="guard-counts-panel" class="admin-panel guard-counts-panel no-print">
    ...
    <div class="admin-body admin-body-single">
      <section class="admin-block">
        ...
        <div class="guard-count-table" role="table" aria-label="Recompte manual de G">
          ...
          <div v-for="row in rows" :key="row.teacherId" class="guard-count-row" role="row">
            ...
            <span role="cell">{{ row.count.released }}</span>
            <label role="cell" :for="`guard-count-${row.teacherId}`">
              <input
                ...
                @change="updateCount(row.teacherId, $event.target.value)"
              />
              <small v-if="saved === row.teacherId">Desat</small>
            </label>
